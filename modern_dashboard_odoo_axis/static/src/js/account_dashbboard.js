    ...
    action_vendor_payment_data : function(value){
            ...
            rpc.query({
                ...
                if(result){
                    ...
                    var dataSet = []
                    for(var i=0;i<res.length;i++){
                        dataSet.push([res[i].partner_name,res[i].amount,res[i].payment_date,'<span class="label label-success">' + '</span>'])
                    }
                    if(dataSet.length > 0){
    ...
    action_customer_record : function(value){
            ...
            rpc.query({
                ...
                if(result){
                    ...
                    var dataSet = []
                    for(var i=0;i<res.length;i++){
                        dataSet.push([res[i].customer_name,res[i].amount,'<span class="label label-success">' + '</span>'])
                    }
                    if(dataSet.length > 0){
    ...
    action_journal_record : function(value){
            ...
            rpc.query({
                ...
                if(result){
                    ...
                    var exp_csv = self.$el.find('#journalcsv');
                    exp_csv.click(function(){        
...
convertcharttocsv: function (args) {
    ...
    graph_account: function() {
        var self = this
        var piectx = this.$el.find('#canvas1')
        Chart.plugins.register({
          beforeDraw: function(chartInstance) {
            var ctx = chartInstance.chart.ctx;
            ctx.fillStyle = "white";
