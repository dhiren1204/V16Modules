...
class CrmInherit(models.Model):
    ...
    def get_crm_list(self):
        ...
        return{
            ...
            'total_revenue': expected_revenue_format,
    ...
    def get_crm_table(self):
        sql_query = """
            SELECT cl.name AS lead_name, cl.expected_revenue AS expected_revenue, cl.probability AS probability
            FROM crm_lead cl   
        ...
        sql_query = """
           SELECT cl.name AS lead_name,cl.probability AS probability
            FROM crm_lead cl
            WHERE cl.probability > 10 limit 10
    ...
    def get_crm_lead(self):
        ...
        query = """
           select * from (
                select  to_char(now(), 'MON-YYYY') as Month ,sum((expected_revenue*probability)/100) as expected_revenue,1 as sorting
                ...
                union
                select  to_char((date_trunc('month', now()) + interval '1 month'), 'MON-YYYY') as Month ,sum((expected_revenue*probability)/100) as expecte_revenue, 2 as sorting
                from crm_lead where date_deadline 
                between (date_trunc('month', now()) + interval '1 month') and ((date_trunc('month', now()) + interval '2 month')- interval '1 day')
                union
                select  to_char((date_trunc('month', now()) + interval '2 month'), 'MON-YYYY') as Month ,sum((expected_revenue*probability)/100) as expected_revenue,3 as sorting
                from crm_lead where date_deadline 
                between (date_trunc('month', now()) + interval '2 month') and ((date_trunc('month', now()) + interval '3 month')- interval '1 day')
                union
                select  to_char((date_trunc('month', now()) + interval '3 month'), 'MON-YYYY') as Month ,sum((expected_revenue*probability)/100) as expected_revenue,4 as sorting
                from crm_lead where date_deadline 
                between (date_trunc('month', now()) + interval '3 month') and ((date_trunc('month', now()) + interval '4 month')- interval '1 day')
                union
                select  to_char((date_trunc('month', now()) + interval '4 month'), 'MON-YYYY') as Month ,sum((expected_revenue*probability)/100) as expected_revenue,5 as sorting
                from crm_lead where date_deadline 
                between (date_trunc('month', now()) + interval '4 month') and ((date_trunc('month', now()) + interval '5 month')- interval '1 day')
                union
                select  to_char((date_trunc('month', now()) + interval '5 month'), 'MON-YYYY') as Month ,sum((expected_revenue*probability)/100) as expected_revenue,6 as sorting
                from crm_lead where date_deadline 
    ...
    def get_crm_team_sale(self):
        ...
        for data in payroll_data:
            for key, value in data['name'].items():
                ...
                payroll_label.append(label_string)
