...
class AccountMoveInherit(models.Model):
    ...
    def get_account_table(self):
        sql_query = """
            SELECT rs.name AS partner_name, ap.amount As amount,ap.payment_type AS payment_type
            FROM account_payment ap INNER JOIN res_partner rs ON (ap.partner_id = rs.id)
            WHERE ap.partner_type = 'customer' limit 10
        ...
        sql_query = """
            SELECT rs.name AS partner_name,ap.amount As amount,CAST(ap.create_date AS DATE) AS payment_date,ap.payment_type AS payment_type
            FROM account_payment ap INNER JOIN res_partner rs ON (ap.partner_id = rs.id)
            WHERE  ap.partner_type = 'supplier'  order by ap.create_date desc limit 10
        ...
        sql_query = """
        SELECT rs.name AS customer_name,ap.amount As amount,ap.create_date
        FROM  res_partner rs 
        INNER JOIN account_payment ap ON (ap.partner_id = rs.id)            
        WHERE ap.partner_type = 'customer' 
        Order by ap.create_date desc, ap.amount desc limit 10    
        ...
        sql_query = """
           SELECT aj.name AS journal_name,aj.type AS types
            FROM  account_journal aj limit 10
        ...
        WHERE ap.state = 'posted' limit 10
        ...
        ap.invoice_date as invoice_date,ap.partner_id as name,rs.name as name,ap.payment_reference as payment
        FROM account_move ap INNER JOIN res_partner rs ON (ap.partner_id = rs.id)
        WHERE ap.state = 'posted' limit 10
    ...
    def get_invoice_order_table(self,option):
        if option == 'today':
            sql_query = """      
                SELECT distinct ap.amount_total AS amount,ap.invoice_origin as origin,
                ap.invoice_date as invoice_date,ap.partner_id as name,rs.name as name,ap.ref as payment
                FROM account_move ap INNER JOIN res_partner rs ON (ap.partner_id = rs.id)
                WHERE ap.state = 'posted' AND ap.invoice_date >= current_date limit 10
            """
        if option == 'monthly':
            sql_query = """      
                SELECT distinct ap.amount_total AS amount,ap.invoice_origin as origin,
                ap.invoice_date as invoice_date,ap.partner_id as name,rs.name as name,ap.ref as payment
                FROM account_move ap INNER JOIN res_partner rs ON (ap.partner_id = rs.id)
                WHERE ap.state = 'posted' AND EXTRACT(month FROM ap.invoice_date::date) = EXTRACT(month FROM CURRENT_DATE)  limit 10
            """
        if option == 'year':
            sql_query = """      
                SELECT distinct ap.amount_total AS amount,ap.invoice_origin as origin,
                ap.invoice_date as invoice_date,ap.partner_id as name,rs.name as name,ap.ref as payment
                FROM account_move ap INNER JOIN res_partner rs ON (ap.partner_id = rs.id)
                WHERE ap.state = 'posted' AND EXTRACT(year FROM ap.invoice_date::date) = EXTRACT(year FROM CURRENT_DATE)  limit 10
            """
        if option == 'all':
            sql_query = """      
                SELECT distinct ap.amount_total AS amount,ap.invoice_origin as origin,
    ...
    def get_customer_payment(self, option):
        ...
        if option == 'today':
            query = """
            SELECT rs.name AS name,MAX(amount)  as  amount
            from account_payment so join res_partner rs on (so.partner_id =rs.id)  
            WHERE so.create_date >= current_date
            group by name
        ...
        if option == 'monthly':
            ...
            SELECT rs.name AS name,MAX(amount)  as  amount
            from account_payment so join res_partner rs on (so.partner_id =rs.id)  
            WHERE EXTRACT(month FROM so.create_date::date) = EXTRACT(month FROM CURRENT_DATE)  
            group by name
            order by name desc limit 8      
            """  
        if option == 'year':
            ...
            SELECT rs.name AS name,MAX(amount)  as  amount
            from account_payment so join res_partner rs on (so.partner_id =rs.id)  
             WHERE EXTRACT(year FROM so.create_date::date) = EXTRACT(year FROM CURRENT_DATE)  
            group by name
            order by name desc limit 8      
            """  
        if option == 'all':
    ...
    def get_supplier_payment(self, option):
        ...
        if option == 'today':
            query = """
              SELECT cl.partner_id AS partner_name,max(amount) as price,rs.name AS partner_name
                ...
                where cl.partner_type ='supplier' AND cl.create_date >= current_date
                group by cl.partner_id,rs.name
        ...
        if option == 'monthly':
            query = """
              SELECT cl.partner_id AS partner_name,max(amount) as price,rs.name AS partner_name
                FROM account_payment cl left JOIN res_partner rs ON (cl.partner_id = rs.id)
                where cl.partner_type ='supplier' AND EXTRACT(month FROM cl.create_date::date) = EXTRACT(month FROM CURRENT_DATE)  
                group by cl.partner_id,rs.name
                order by cl.partner_id limit 10
            """
        if option == 'year':
            query = """
              SELECT cl.partner_id AS partner_name,max(amount) as price,rs.name AS partner_name
                FROM account_payment cl left JOIN res_partner rs ON (cl.partner_id = rs.id)
                where cl.partner_type ='supplier' AND EXTRACT(year FROM cl.create_date::date) = EXTRACT(year FROM CURRENT_DATE)  
                group by cl.partner_id,rs.name
                order by cl.partner_id limit 10
            """
        if option == 'all':
