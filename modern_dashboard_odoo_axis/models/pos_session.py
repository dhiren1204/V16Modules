...
#             tz = timezone(tz)
    ...
    def payment_by_journal_pie_chart(self, company_id):
        ...
        method_ids = self.env['pos.payment.method'].search(domain)
        payment_ids = self.env['pos.payment'].search([('payment_method_id', 'in', method_ids.ids )]).ids
        SQL = ""
        if len(payment_ids) > 1:
            SQL = "pp.id IN  %s" % str(tuple(payment_ids))
        else:
            if len(payment_ids) > 0:
                SQL = "pp.id =  %s" % payment_ids[0]
        if len(payment_ids) > 0:
            sql_query = """SELECT SUM(amount) AS amount, pm.name AS journal
                            FROM pos_payment AS pp
                            INNER JOIN pos_payment_method AS pm ON pm.id = pp.payment_method_id
                            WHERE %s
                            GROUP BY journal
                            """ % str(SQL)
    ...
    def get_journal_line_chart_data(self, start_date, end_date, journal, company_id):
        ...
        SQL_domain = ""
        if len(payment_ids) > 1:
            SQL_domain = "pp.id IN  %s" % str(tuple(payment_ids))
        else:
            if len(payment_ids) > 0:
                SQL_domain = "pp.id =  %s" % payment_ids[0]
        if len(payment_ids) > 0:
            sql_query = """SELECT SUM(amount) AS amount, 
                            extract(day from pp.create_date AT TIME ZONE 'UTC' AT TIME ZONE '%s') AS time_duration,
                            pm.name AS journal, 
                            pm.id
                            FROM pos_payment AS pp
                            INNER JOIN pos_payment_method AS pm ON pm.id = pp.payment_method_id
                            WHERE %s
                            AND pp.create_date >= '%s'
                            AND pp.create_date <= '%s' %s
                            GROUP BY time_duration, journal, pm.id ORDER BY extract(day from pp.create_date AT TIME ZONE 'UTC' AT TIME ZONE '%s') ASC
                            """ % (current_time_zone, SQL_domain, s_date, e_date, sql, current_time_zone)
            self._cr.execute(sql_query)
            # journal_data = self._cr.dictfetchall()
            # print("journal data",sql_query)
            # return journal_data
