...
class MailRenderMixin(models.AbstractModel):
    ...
    def _replace_local_links(self, html, base_url=None):
        ...
        # brand = self.env['ir.config_parameter'].sudo().get_param('app_system_name', 'BrainPack')
        brand = self.env.company.app_system_name or 'BrainPack'
        message = re.sub(
            r"""(Powered by\s(.*)Odoo</a>)""", "<div>Powered by <a href='https://www.brainpack.io/'>"+brand+"</a></div>", message
        )
        message = re.sub(
            r"""(Odoo)""", brand, message
        )
